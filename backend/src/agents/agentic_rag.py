...
from guardrails import agent_guardrail, retrieval_guardrail, tool_guardrail, describe_block
...
"""
    ...
    # ------------------------------------------------------------------
    def run(self, question: str, history: str | None = None, session_id: str | None = None) -> dict:
        with metrics.timed_agent_step("agentic_rag", "run"):
            ...
            def _tokens() -> tuple[int, int]:
                return (
                    sum(u["input_tokens"] for u in self._usage_log),
                    sum(u["output_tokens"] for u in self._usage_log),
                )

            subquestions = self.analyze_query(question)

            # ── Agent Plan Validation (sub-questions) ─────────────────
            plan_result = agent_guardrail.check_plan(question, subquestions=subquestions)
            audit_logger.log("agent_guardrail:plan_subquestions", plan_result, session_id=session_id)
            subquestions = plan_result.data["subquestions"]

            retrieved = self.retrieve(subquestions)
            sources   = self._build_sources(retrieved)

            # ── Retrieval Guardrail ────────────────────────────────────
            retrieval_result = retrieval_guardrail.check_retrieval(question, sources)
            audit_logger.log("retrieval_guardrail", retrieval_result, session_id=session_id)
            if not retrieval_result.allowed:
                input_tokens, output_tokens = _tokens()
                return {
                    "answer":            "I couldn't find any relevant contract content to answer this question.",
                    "task":              "general",
                    "task_reason":       retrieval_result.reason,
                    "draft_answer":      "",
                    "is_grounded":       False,
                    "validation_issues": [retrieval_result.reason],
                    "subquestions":      subquestions,
                    "retrieved":         retrieved,
                    "sources":           [],
                    "input_tokens":      input_tokens,
                    "output_tokens":     output_tokens,
                    "guardrail_blocked": "retrieval_guardrail",
                    "blocked_info":      describe_block(retrieval_result),
                }
            ...
            # ── Agent Plan Validation (task route) ─────────────────────
            plan_task_result = agent_guardrail.check_plan(question, task=route.task)
            audit_logger.log("agent_guardrail:plan_task", plan_task_result, session_id=session_id)
            validated_task = plan_task_result.data["task"]
            task_value = validated_task.value if hasattr(validated_task, "value") else str(validated_task)

            logger.info("Task routed: %s — %s", task_value, route.reason)

            # ── Tool Execution Guardrail (specialist agent) ────────────
            tool_result = tool_guardrail.guarded_tool_call(
                "specialist_agent", self.run_specialist_agent,
                validated_task, question, context, history_section,
            )
            audit_logger.log("tool_guardrail:specialist_agent", tool_result, session_id=session_id)
            if not tool_result.allowed:
                input_tokens, output_tokens = _tokens()
                return {
                    "answer":            "I ran into an internal error while analyzing this question. Please try again.",
                    "task":              task_value,
                    "task_reason":       route.reason,
                    "draft_answer":      "",
                    "is_grounded":       False,
                    "validation_issues": [tool_result.reason],
                    "subquestions":      subquestions,
                    "retrieved":         retrieved,
                    "sources":           sources,
                    "input_tokens":      input_tokens,
                    "output_tokens":     output_tokens,
                    "guardrail_blocked": "tool_guardrail",
                    "blocked_info":      describe_block(tool_result),
                }
            draft_answer = tool_result.data
