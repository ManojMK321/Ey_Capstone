...
from guardrails import document_guardrail
...
async def upload_documents(request: Request) -> UploadResponse:
    ...
    for file in files:
        ...
        try:
            ...
            # ── Document Guardrail ──────────────────────────────────────
            doc_guard = document_guardrail.check_document(file.filename, file.content_type, data)
            audit_logger.log("document_guardrail", doc_guard, session_id=session_id, extra={"filename": file.filename})
            if not doc_guard.allowed:
                errors.append({"filename": file.filename, "reason": doc_guard.reason})
                metrics.UPLOAD_ERRORS.labels(file_type=file_type, error_type="document_guardrail").inc()
                continue
            data = doc_guard.data
...
async def upload_stream(request: Request):
    ...
    async def generate():
        ...
        try:
            ...
            if not doc_guard.allowed:
                metrics.UPLOAD_ERRORS.labels(file_type=file_type, error_type="document_guardrail").inc()
                yield f"data: {json.dumps({'error': doc_guard.reason})}\n\n"
                return
            data = doc_guard.data
