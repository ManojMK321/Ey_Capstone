...
# eval/ lives under backend/ (shared with src/observability/langsmith_eval.py)
# but this Streamlit process runs from frontend/, so reach across the
# backend/frontend boundary explicitly instead of relying on cwd.
_ROOT     = Path(__file__).parents[1]
_EVAL_DIR = _ROOT / "backend" / "eval"
...
/* ── suggested-question chips (security notice) ── */
.suggestion-chip {
  display:inline-block;
  background:rgba(100,160,255,.08); color:#7aa8ff;
  border:0.5px solid rgba(100,160,255,.25); border-radius:6px;
  padding:4px 10px; font-size:11px; margin:0 4px 4px 0;
}
...
def _risk_color(score) -> str:
    if score is None:
        return "#666666"
    return "#F44336" if score >= 70 else ("#FF9800" if score >= 40 else "#4CAF50")
...
def _security_card_html(threat_type, threat_detail, risk_score, action="Request Blocked") -> str:
    color = _risk_color(risk_score)
    threat_line = threat_type or "Policy Violation"
    if threat_detail:
        threat_line += f" ({threat_detail})"
    score_display = f"{risk_score}/100" if risk_score is not None else "—"
    suggestions = "".join(
        f'<span class="suggestion-chip">{icon} {label}</span>' for icon, label in _SUGGESTED_PROMPTS
    )
    return f"""
<div style="background:#1a1a1a;border:0.5px solid {color}55;border-radius:10px;
            padding:16px 18px;margin-bottom:4px;max-width:440px;">
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;">
    <span style="font-size:18px;">🛡️</span>
    <span style="font-size:13px;font-weight:600;color:#f0f0f0;letter-spacing:.2px;">DocIQ Security Center</span>
  </div>
  <div style="display:grid;grid-template-columns:auto 1fr;row-gap:7px;column-gap:14px;font-size:12px;">
    <span style="color:#888;">Threat Detected</span><span style="color:{color};font-weight:600;">{threat_line}</span>
    <span style="color:#888;">Risk Score</span><span style="color:{color};font-weight:700;">{score_display}</span>
    <span style="color:#888;">Action Taken</span><span style="color:#f0f0f0;">{action}</span>
    <span style="color:#888;">Status</span><span style="color:#5ac878;">Document Protected ✅</span>
  </div>
  <div style="margin-top:14px;padding-top:12px;border-top:0.5px solid #262626;">
    <div style="font-size:10px;color:#888;font-weight:600;text-transform:uppercase;
                letter-spacing:.06em;margin-bottom:8px;">You may ask</div>
    <div>{suggestions}</div>
  </div>
</div>"""
    ...
    for msg_idx, msg in enumerate(messages):
        ...
        if msg.get("is_blocked"):
            with st.chat_message(msg["role"], avatar="🛡️"):
                st.markdown(
                    _security_card_html(
                        msg.get("threat_type"), msg.get("threat_detail"),
                        msg.get("risk_score"), msg.get("action", "Request Blocked"),
                    ),
                    unsafe_allow_html=True,
                )
            continue
            ...
            try:
                ...
                if resp.ok:
                    ...
                    if data.get("blocked"):
                        st.markdown(
                            _security_card_html(
                                data.get("threat_type"), data.get("threat_detail"),
                                data.get("risk_score"), "Content Flagged",
                            ),
                            unsafe_allow_html=True,
                        )
                        session["messages"].append({
                            "role": "assistant", "content": "", "is_blocked": True,
                    ...
                    if isinstance(detail, dict) and "blocked" in detail:
                        st.markdown(
                            _security_card_html(
                                detail.get("threat_type"), detail.get("threat_detail"),
                                detail.get("risk_score"), detail.get("action", "Request Blocked"),
                            ),
                            unsafe_allow_html=True,
                        )
                        session["messages"].append({
                            ...
                            "threat_detail": detail.get("threat_detail"),
                            "risk_score":    detail.get("risk_score"),
                            "action":        detail.get("action", "Request Blocked"),
                        })
                    else:
